...
impl RedisPubSub {
    ...
    pub async fn publish(&self, msg: &PubSubMessage) -> Result<(), ShaperailError> {
        ...
        let payload = serde_json::to_string(msg).map_err(|e| {
            ShaperailError::Internal(format!("Failed to serialize pub/sub message: {e}"))
        })?;

        let redis_channel = Self::redis_channel(&msg.channel);
        conn.publish::<_, _, ()>(&redis_channel, &payload)
            .await
            .map_err(|e| ShaperailError::Internal(format!("Redis publish failed: {e}")))?;
    ...
    async fn subscriber_loop(
        ...
        let client = redis::Client::open(redis_url)
            .map_err(|e| ShaperailError::Internal(format!("Redis client creation failed: {e}")))?;

        let mut pubsub_conn = client.get_async_pubsub().await.map_err(|e| {
            ShaperailError::Internal(format!("Redis pub/sub connection failed: {e}"))
        })?;

        pubsub_conn
            .subscribe(redis_channel)
            .await
            .map_err(|e| ShaperailError::Internal(format!("Redis subscribe failed: {e}")))?;
        ...
        while let Some(msg) = msg_stream.next().await {
            ...
            let server_msg = shaperail_core::WsServerMessage::Broadcast {
