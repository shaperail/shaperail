...
use shaperail_core::{EventSubscriber, EventTarget, EventsConfig, ShaperailError};
...
impl EventEmitter {
    ...
    pub async fn emit(
        ...
        let payload_json = serde_json::to_value(&payload)
            .map_err(|e| ShaperailError::Internal(format!("Failed to serialize event: {e}")))?;
    ...
    async fn dispatch_to_target(
        ...
        match target {
            ...
            EventTarget::Hook { name } => {
                ...
                self.job_queue
                    .enqueue("shaperail:hook_execute", hook_payload, JobPriority::Normal)
                    .await?;
            }
        }
        Ok(())
