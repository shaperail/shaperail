...
fn build_operation(
    ...
    if let Some(hooks) = &ep.hooks {
        if !hooks.is_empty() {
            operation.insert("x-steelapi-hooks".to_string(), serde_json::json!(hooks));
        }
    }
    if let Some(events) = &ep.events {
        if !events.is_empty() {
            operation.insert("x-steelapi-events".to_string(), serde_json::json!(events));
        }
    }
...
mod tests {
    ...
    use steel_core::{AuthRule, CacheSpec, FieldSchema, FieldType, HttpMethod, PaginationStyle};

    fn test_config() -> ProjectConfig {
        ProjectConfig {
            project: "test-api".to_string(),
            port: 3000,
            workers: steel_core::WorkerCount::Auto,
            database: None,
            cache: None,
            auth: None,
            storage: None,
    ...
    fn vendor_extensions() {
        ...
        let create_op = &spec["paths"]["/users"]["post"];
        assert_eq!(
            create_op["x-steelapi-hooks"],
            serde_json::json!(["validate_org"])
        );
        assert_eq!(
            create_op["x-steelapi-events"],
