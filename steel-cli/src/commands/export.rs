...
pub fn run_openapi(output: Option<&Path>) -> i32 {
    ...
    let spec = steel_codegen::openapi::generate(&config, &resources);

    match output {
        Some(path) => {
            let content = if path.extension().is_some_and(|e| e == "yaml" || e == "yml") {
                match steel_codegen::openapi::to_yaml(&spec) {
                    Ok(s) => s,
                    Err(e) => {
                ...
                match steel_codegen::openapi::to_json(&spec) {
                    Ok(s) => s,
                    Err(e) => {
        ...
        None => match steel_codegen::openapi::to_json(&spec) {
            Ok(s) => println!("{s}"),
            Err(e) => {
                eprintln!("Error serializing JSON: {e}");
                return 1;
...
pub fn run_sdk(lang: &str, output: Option<&Path>) -> i32 {
    ...
    let spec = steel_codegen::openapi::generate(&config, &resources);
    let files = steel_codegen::typescript::generate_from_spec(&spec);
