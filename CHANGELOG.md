...
### Added
- **`shaperail explain` now prints a `Validations:` section** listing per-field constraint summaries (required, unique, min/max, format, enum values, default, sensitive, transient). Also prints an `OpenAPI fragments:` section with per-endpoint request shape, response status codes, and auth roles.
- **`shaperail explain --format json`** emits a stable JSON representation of the resource (routes, table, relations, validations, OpenAPI fragments, indexes). Field names are part of the CLI contract. See `docs/cli-reference.md` for the full shape.
...
- **`Diagnostic` carries optional `span`, `severity`, and `doc_url`.** `shaperail check --json` now includes `severity` (`error` | `warning` | `info`) and `doc_url` (a permanent URL into `https://shaperail.io/errors/<code>.html`) for every diagnostic. With the `saphyr-spans` cargo feature enabled, root-level codes (SR001-SR005) also carry `span` (file/line/col/end\_line/end\_col, 1-indexed, exclusive end). All fields are additive — JSON consumers tolerant of unknown fields are unaffected.
- **Central diagnostic registry.** `shaperail_codegen::diagnostics::registry::REGISTRY` is now the single source of truth for SR\* codes (43 entries), severity, and one-line summaries. `Diagnostic::error("SR042", ...)` consults the registry for severity and computes `doc_url`. SR000 (YAML parse error) joined the registry; SR100 (legacy INT/BIGINT drift warning) is still constructed directly in the CLI.
- **`saphyr-spans` cargo feature on `shaperail-codegen`.** Adds an opt-in saphyr-based YAML parser that produces a `SpanMap` alongside the parsed `ResourceDefinition`. Off by default; CI runs the workspace test suite under both feature configurations.
- **Per-error reference pages.** Every `SR*` code now has a permanent reference page under `docs/errors/<code>.md` with severity, summary, an example that triggers it, and the canonical fix. The index lives at `docs/errors/index.md` and is linked from `doc_url` on every diagnostic. A new CI test (`shaperail-codegen/tests/diagnostic_doc_pages.rs`) asserts the registry and the doc tree stay in sync.
- **Recipe library (4 of 8).** Standalone Cargo example crates under `examples/recipes/` showing canonical resource patterns: `paginated_list_with_filters` (cursor + filters + sort + cache-with-invalidate), `file_upload` (multipart with size and MIME enforcement), `multi_tenant_rls` (organizations + documents with `tenant_key`), `rate_limited_public` (public-create + admin-read with per-IP rate limit). Each ships with a YAML resource, README explaining when to use and when not to, and an integration test scaffold (some tests are `#[ignore]`'d pending `TestServer` helper expansion). Remaining 4 recipes ship in v0.16.0 with the declarative-primitives work.
