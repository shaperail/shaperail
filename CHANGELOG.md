...
### Breaking
- **`AppState::new` now requires the parsed proxy configuration as its third argument.** Existing bootstraps must change `AppState::new(pool, resources)` to `AppState::new(pool, resources, config.proxy.as_ref())`; isolated tests and direct-only deployments can pass `None`. Requiring an explicit value prevents `proxy.trusted_proxies` from being parsed successfully but silently ignored by applications generated with older Shaperail versions.
...
- **Codegen output type:** struct fields for columns with `default:`, `generated:`, `required:`, or `primary: true` now codegen as `T` instead of `Option<T>`. Mechanical fix: remove `.unwrap_or(default)` / `.unwrap()` at call sites. `nullable: true` always wins. The rule and full matrix are in `agent_docs/codegen-patterns.md` and `docs/resource-guide.md`.
- **`ControllerSpec.before` / `after`:** type changed from `Option<String>` to `Option<HookList>` (an untagged `String | Vec<String>`). Affects code that reads the parsed AST directly (rare); YAML stays source-compatible for the scalar form.
- **`ControllerSpec::wasm_before_path` / `wasm_after_path` removed.** These returned the first `wasm:` entry in a hook chain, silently ignoring non-WASM hooks. Use `before_names()` / `after_names()` and inspect each entry; `has_wasm_before()` / `has_wasm_after()` remain.

### Added
- **Trusted reverse-proxy client IP resolution.** `shaperail.config.yaml` now accepts `proxy.trusted_proxies` as explicit IPv4/IPv6 CIDR ranges. Forwarding headers remain untrusted by default; trusted chains use the canonical `X-Forwarded-For` header and are evaluated from right to left. `Context::client_ip()`, `AppState::client_ip(&req)`, WASM controller context, request logging, and per-IP rate limiting now share the normalized result.
- **`shaperail explain` validates intent without running codegen.** Two new sections in the human-readable text output: `Validations:` lists per-field constraint summaries (required, unique, min, max, format, enum values, default, sensitive, transient — one compact line per field), and `OpenAPI fragments:` lists per-endpoint request shape, response status codes (including 401/403 when auth is set, 422 when the endpoint takes a body, 404 for get/update/delete), and auth roles. New `--format json` flag emits the same information as a stable JSON document with documented top-level keys (`resource`, `version`, `db`, `tenant_key`, `routes`, `table`, `relations`, `validations`, `openapi`, `indexes`); field names are part of the CLI contract and pinned by an integration test that walks the documented schema (`shaperail-cli/tests/cli_tests.rs::explain_format_json_matches_documented_schema`). Motivation: an LLM authoring a resource needs to verify its YAML expanded to the right routes/validations/openapi shape before running `cargo build`. Without this, the verification loop went all the way through codegen and the Rust compiler; with `explain --format json` it's a single CLI call. Migration: none; the full OpenAPI 3.1 spec still lives in `shaperail export openapi`.
- **`shaperail check` warns on integer-column drift after v0.13.0 upgrade (`SR100`).** The check command now scans every `migrations/*.sql` and emits a non-fatal warning for any column whose latest committed migration declares `INTEGER` but whose schema field is `type: integer` (which now emits `BIGINT` post-v0.13.0). The warning names the offending migration and tells the author exactly which `ALTER TABLE … TYPE BIGINT` statement to add. Cleared automatically by a follow-up ALTER migration. JSON output mode includes warnings as full diagnostics.
- **Upgrade guide for v0.13.0 (`agent_docs/upgrade-0.13.md` + `docs/upgrade-0.13.md`).** Covers the steps required to take a v0.12.x app to v0.13.0 cleanly: bumping deps, re-running codegen, ALTER migrations for `type: integer` columns, the `auth.id` → `auth.sub` rename, and the `SR100` drift warning that flags missing ALTERs at check time.
...
- **Hook chains:** `controller.before` / `controller.after` accept arrays of hook names. Hooks run in declaration order, sequentially; the first `Err(_)` short-circuits the chain. WASM and Rust hooks may be mixed.
- **`ctx.path_params`:** new `HashMap<String, String>` field on `ControllerContext`, plus `ctx.path_param(name) -> Option<&str>` helper. Populated from URL path segments (`:id`, etc.) before any hook runs. Empty for `list` and `create`. Wired for `update`, `delete`, and custom endpoints; not yet for `get` or `update_upload` (no before-hook dispatch on those handlers).
- **`pub mod resources` aggregator:** `generated/mod.rs` now emits a `#[doc(hidden)] pub mod resources` block re-exporting every `<name>_controller`. Library projects add `pub use generated::resources;` to `lib.rs` to expose controller helpers to integration tests.
- **Validator rule SR063:** rejects empty `before: []` / `after: []` arrays.
- **JSON Schema export:** `ControllerSpec.before` / `after` now allow string OR non-empty array of strings, matching the new YAML grammar.
- **`Diagnostic` struct gains optional `span`, `severity`, and `doc_url` fields.** `shaperail check --json` now includes `severity` (`error` | `warning` | `info`) and `doc_url` (a permanent URL into `https://shaperail.io/errors/<code>.html`) for every diagnostic. With the new `saphyr-spans` cargo feature enabled, root-level codes (SR001-SR005) additionally carry `span` (`{ file, line, col, end_line, end_col }`, 1-indexed, UTF-8 byte columns, exclusive end positions per `shaperail-codegen/src/diagnostics/types.rs`). All fields are additive — existing JSON consumers tolerant of unknown fields are unaffected. Motivation: an LLM consuming `shaperail check --json` previously had to scan source code to find a code's stable URL and could not point at the offending source position; with the new fields it self-corrects without a second query. Migration: callers of `shaperail_codegen::diagnostics::Diagnostic` should use `Diagnostic::error("SR042", error, fix, example)` rather than the public struct literal — the constructor is now the only way to populate `severity`/`doc_url` consistently from the registry; debug builds `debug_assert!` if the code is missing from the registry.
- **Central diagnostic registry at `shaperail_codegen::diagnostics::registry`.** All 43 `SR*` codes emitted by `shaperail-codegen` are listed in `REGISTRY: &[RegistryEntry]` with their stable severity and one-line summary. `Diagnostic::error(code, ...)` looks up severity and computes `doc_url` from this table. SR000 (YAML parse error, emitted by `shaperail-cli/src/commands/check.rs`) joined the registry so the constructor's debug-assert stays clean; SR100 (legacy INT/BIGINT drift warning) remains CLI-only because it's emitted as raw JSON without going through `Diagnostic::error`. Motivation: the per-error doc pages, the upcoming sectioned LLM Guide, and any future schema-introspection tooling all need a single source of truth for what diagnostic codes exist; previously codes were scattered across emission-site string literals. Migration: none required — emission sites switched to the new constructor in this release. A new CI test (`shaperail-codegen/tests/diagnostic_doc_pages.rs`) asserts the registry and `docs/errors/<code>.md` stay in sync, so adding a new diagnostic forces a doc page in the same commit.
- **`saphyr-spans` cargo feature on `shaperail-codegen` and `shaperail-cli`.** Off by default; opt in with `cargo install shaperail-cli --features saphyr-spans`. Enables a saphyr-based parser that tracks source positions through validation; `shaperail check --json` then emits a `span` object on each root-level diagnostic. Default builds use serde_yaml as before — no behavior change. Motivation: `serde_yaml` exposes no API for source positions, but LLMs (and IDEs) need them to correct mistakes by line/column rather than by re-parsing on their own. Saphyr ships behind a flag for one patch cycle so we can verify parity in CI (the workspace test matrix runs both configurations on every PR) before flipping the default in v0.16.0. Migration: callers of `shaperail_codegen::parser::parse_resource_file` are unaffected; callers wanting spans should use `shaperail_codegen::parser::diagnose_file(path) -> Result<(ResourceDefinition, Vec<Diagnostic>), ParseError>`, which transparently uses the saphyr path when the feature is on and falls back to serde_yaml otherwise.
- **`shaperail check` reuses the parsed `ResourceDefinition` between diagnose and drift-detection.** Previously the CLI parsed each resource file twice — once for `diagnose_resource` and once for the cross-resource SR100 integer-drift check. The new `shaperail_codegen::parser::diagnose_file` returns `(ResourceDefinition, Vec<Diagnostic>)`, so the CLI passes the parsed value straight into drift detection. On a directory of N resource files this halves the parse work. No user-visible behavior change.
- **Per-error reference pages at `docs/errors/SR<NNN>.md`.** Every diagnostic code now has a permanent reference page with severity, summary, an example YAML that triggers it, and the canonical fix. The `doc_url` field on every JSON diagnostic links into `https://shaperail.io/errors/<code>.html`; the index at `docs/errors/index.md` lists all 43 codes. Motivation: an LLM that hits a diagnostic should be able to fetch one focused page rather than search the entire LLM Guide; the URL is now part of the diagnostic itself, so there is no lookup step. The `diagnostic_doc_pages` integration test ensures pages stay in sync with the registry — a new code without a corresponding page fails CI.
- **Recipe library at `examples/recipes/` (4 of 8).** Each recipe is a self-contained reference YAML the LLM is told to imitate verbatim for a specific pattern: `paginated_list_with_filters` (cursor + multi-column filters + sort + cache-with-invalidate-on-write), `file_upload` (multipart with size and MIME enforcement via the `upload:` endpoint key), `multi_tenant_rls` (organizations + documents pair demonstrating the 404-not-403 invariant for cross-tenant access via `tenant_key`), `rate_limited_public` (public-create + admin-read endpoint pair guarded by per-IP rate limiting). Each recipe ships with a YAML resource, a README explaining when to use the pattern and when not to, and an integration-test scaffold (some tests `#[ignore]`'d pending TestServer helper expansion in a future release). All four resources pass `shaperail check` with zero diagnostics. Motivation: codegen is deterministic, so an LLM cannot pattern-match Shaperail code from training data — it needs canonical complete examples to imitate. Remaining 4 recipes (`soft_delete_with_audit`, `parent_child_with_cascade`, `approval_workflow`, `idempotent_webhook_receiver`) ship in v0.16.0 once the declarative primitives they depend on land.

### Fixed
- **Per-IP behavior no longer disagrees behind a reverse proxy.** Rate limiting previously always used the socket peer while maintained audit-log examples trusted raw caller headers, allowing every proxied request to share one bucket and exposing audit records to spoofing. Both now use the same trusted-proxy resolver, and the public rate-limit recipe explicitly persists `ctx.client_ip()` through a before-controller.
- **Project commands now load the project `.env` before dispatch.** Explicit process environment variables still take precedence, `init` and `doctor` remain independent of project configuration, and malformed dotenv files report the filename and parse error instead of failing later with a misleading missing-variable message.
- **Generated migration nullability now matches generated Rust models.** Required, generated, primary-key, and defaulted fields emit `NOT NULL` across PostgreSQL, MySQL, and SQLite; nullable primary keys are rejected because database primary keys cannot admit `NULL`.
- **The default scaffold's ownership model is internally consistent.** Posts now use the built-in `created_by` ownership field, populate it from the authenticated subject in a before-controller, and no longer expose the server-owned value as client input.
- **Scaffolded AI guidance and JSON Schema now describe the shipped framework.** Generated instructions use `shaperail llm-context`, current field types and controller APIs, and a resource-local `.schema.json` reference. The deterministic public schema uses the canonical `shaperail.io` identifier and includes constrained array items, transient fields, rate limits, subscribers, and custom handlers.
- **Maintained examples now match current validation, auth, and migration behavior.** Controller examples inject server-owned values without panicking, bind path/input UUIDs correctly, use opaque JWT subjects safely, and declare soft-delete fields consistently. Follow-up migrations preserve SQLx checksums for existing example databases, and checked-in resource YAML has repository-wide validation coverage.
