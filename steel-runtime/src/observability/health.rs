...
async fn redis_health_check(pool: &deadpool_redis::Pool) -> Result<(), SteelError> {
    let mut conn = pool
        .get()
        .await
        .map_err(|e| SteelError::Internal(format!("Redis health check failed: {e}")))?;
    let _: String = redis::cmd("PING")
        .query_async(&mut conn)
        .await
        .map_err(|e| SteelError::Internal(format!("Redis PING failed: {e}")))?;
    Ok(())
}
