...
use sqlx::{PgPool, Row};
use steel_core::{FieldSchema, FieldType, ResourceDefinition, SteelError};
...
impl<'a> ResourceQuery<'a> {
    ...
    fn row_to_json(&self, row: &PgRow) -> Result<serde_json::Value, SteelError> {
    ...
    pub async fn find_all(
        ...
        sort: &SortParam,
        page: &PageRequest,
    ) -> Result<(Vec<ResourceRow>, serde_json::Value), SteelError> {
            ...
            PageRequest::Cursor { after, limit } => {
                let decoded_cursor = if let Some(cursor_str) = after {
                    ...
                    let id = uuid::Uuid::parse_str(&id_str).map_err(|_| {
                        SteelError::Validation(vec![steel_core::FieldError {
    ...
    pub async fn insert(
        ...
        rows.into_iter()
            .next()
            .ok_or_else(|| SteelError::Internal("Insert returned no rows".to_string()))
    }
    ...
    pub async fn update_by_id(
        ...
        if set_clauses.is_empty() {
            return Err(SteelError::Validation(vec![steel_core::FieldError {
        ...
        let rows = self.execute_query(&sql, &bind_values).await?;
        rows.into_iter().next().ok_or(SteelError::NotFound)
    }

    /// Soft-delete a record by setting `deleted_at` to now.
    pub async fn soft_delete_by_id(&self, id: &uuid::Uuid) -> Result<ResourceRow, SteelError> {
        let pk = self.primary_key();
        let sql = format!(
    ...
    pub async fn hard_delete_by_id(&self, id: &uuid::Uuid) -> Result<ResourceRow, SteelError> {
        let pk = self.primary_key();
        let sql = format!(
...
fn extract_column_value(
    ...
    let map_err = |e: sqlx::Error| SteelError::Internal(format!("Column '{name}' error: {e}"));
