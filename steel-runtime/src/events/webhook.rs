...
/// Signature format: `X-Steel-Signature: sha256=<hex-digest>`
#[derive(Clone)]
...
impl WebhookDispatcher {
    ...
    pub fn from_env(secret_env: &str, timeout_secs: u64) -> Result<Self, SteelError> {
        let secret = std::env::var(secret_env).map_err(|_| {
            SteelError::Internal(format!("Webhook secret env var '{secret_env}' not set"))
        })?;
    ...
    ) -> Result<WebhookRequest, SteelError> {
        let body = serde_json::to_vec(payload)
            .map_err(|e| SteelError::Internal(format!("Failed to serialize webhook body: {e}")))?;
...
impl WebhookRequest {
    /// Returns the `X-Steel-Signature` header value.
