...
use steel_core::{InboundWebhookConfig, SteelError};
...
/// - `X-Steel-Signature: sha256=<hex>` (SteelAPI format)
/// - `X-Hub-Signature-256: sha256=<hex>` (GitHub format)
/// - `Stripe-Signature: t=<ts>,v1=<sig>` (Stripe format)
pub fn verify_signature(req: &HttpRequest, body: &[u8], secret: &str) -> Result<(), SteelError> {
    // Try SteelAPI signature header
    if let Some(sig_header) = req.headers().get("X-Steel-Signature") {
        let sig_str = sig_header.to_str().map_err(|_| SteelError::Unauthorized)?;
        let hex_sig = sig_str
            .strip_prefix("sha256=")
